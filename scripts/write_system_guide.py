...
doc1_content = """# Enterprise AI Credit Scoring & Decision Engine — Production Architecture Specification
...
$$PD_{\text{raw}}(\mathbf{x}) = \sigma(F_M(\mathbf{x})) = \frac{1}{1 + e^{-F_M(\mathbf{x})}}$$
...
* $\frac{\partial PD}{\partial (\text{Income})} \le 0$ (Default probability must not increase as verified income increases).
* $\frac{\partial PD}{\partial (\text{FOIR})} \ge 0$ (Default probability must not decrease as debt obligations increase).
* $\frac{\partial PD}{\partial (\text{DPD})} \ge 0$ (Default probability must not decrease as days past due increase).
...
$$\mathbf{z}(\mathbf{x}) = \left[ PD_{\text{LGBM}}(\mathbf{x}), PD_{\text{XGB}}(\mathbf{x}), PD_{\text{Logit}}(\mathbf{x}), P_{01}(\mathbf{x}), \dots, P_{11}(\mathbf{x}) \right]$$

The ensembled default probability is computed via a regularized Logistic Stacking Meta-Learner:

$$PD_{\text{ens}}(\mathbf{x}) = \sigma\left(\mathbf{w}^T \mathbf{z}(\mathbf{x}) + b\right) = \frac{1}{1 + e^{-(\mathbf{w}^T \mathbf{z}(\mathbf{x}) + b)}}$$
...
$$PD_{\text{cal}} = \text{IsotonicFit}\left(PD_{\text{ens}} \to \text{Observed Default Frequency by Score Decile}\right)$$

#### Calibration Quality Metrics:
1. **Brier Score:**
   $$\text{Brier} = \frac{1}{N} \sum_{i=1}^N (PD_{\text{cal}, i} - y_i)^2$$
2. **Hosmer–Lemeshow Test Statistic:** Evaluated across deciles of risk:
   $$H = \sum_{g=1}^{10} \frac{(O_g - N_g \bar{\pi}_g)^2}{N_g \bar{\pi}_g (1 - \bar{\pi}_g)} \sim \chi_8^2$$
...
$$\text{Odds} = \frac{1 - PD_{\text{cal}}}{PD_{\text{cal}}}$$
...
#### Production Anchors:
* $\text{BaseScore} = 660$ at $\text{BaseOdds} = 15:1$ ($PD \approx 6.25\%$)
* $\text{PDO} = 40$ (Every 40 score points doubles the odds of non-default)

$$\text{Factor} = \frac{40}{\ln(2)} = 57.7078$$

$$\text{Offset} = 660 - 57.7078 \cdot \ln(15) = 660 - 156.273 = 503.727$$

$$\text{Score} = \text{clip}\left(503.73 + 57.71 \cdot \ln(\text{Odds}), 300, 900\right)$$

*Example Calculation:* For an applicant with calibrated $PD = 3.0\%$:
$$\text{Odds} = \frac{1 - 0.03}{0.03} = 32.333$$
$$\text{Score} = 503.73 + 57.71 \cdot \ln(32.333) = 503.73 + 57.71 \cdot (3.476) \approx 704$$
...
* **Lifetime ECL (IFRS 9 Stage 2 & Stage 3):**
  $$\text{Lifetime ECL} = \sum_{t=1}^T \frac{\text{PD}_t \times \text{LGD}_t \times \text{EAD}_t}{(1 + \text{EIR})^t}$$
...
| **Brier Score** | $\frac{1}{N} \sum (\text{PD}_i - y_i)^2$ | $\le 0.90 \times \text{Naive Base Rate}$ | **0.2173** |
| **Population Stability (PSI)**| $\sum (Actual\% - Expected\%) \cdot \ln(Actual\% / Expected\%)$ | $< 0.10$ (Green) / $0.10 - 0.25$ (Amber) | **0.038 (Green)** |
| **Disparate Impact Ratio**| $\frac{\text{Approval Rate}_{\text{Protected}}}{\text{Approval Rate}_{\text{Reference}}}$ | $\ge 0.80$ (Four-Fifths Rule compliance) | **0.894 (Fair / Unbiased)** |
...
$$\text{Approved Limit} = \min\left( \text{AffordabilityLimit}, \text{RiskLimit}, \text{PolicyCap}, \text{ProgressionCap} \right)$$

1. **Affordability Limit:**
   $$\text{AffordabilityLimit} = \text{Max Disposable EMI} \times \left[ \frac{1 - (1 + r)^{-n}}{r} \right]$$
...
| :--- | :---: | :---: | :--- |
| `lgb_income_p10.joblib` | JOBLIB | 1.49 MB | LightGBM Regressor (Quantile $\alpha=0.10$, Pinball Loss) |
| `lgb_income_p50.joblib` | JOBLIB | 1.50 MB | LightGBM Regressor (Quantile $\alpha=0.50$, Pinball Loss) |
| `lgb_income_p90.joblib` | JOBLIB | 1.48 MB | LightGBM Regressor (Quantile $\alpha=0.90$, Pinball Loss) |
...
print("Updated SYSTEM_ARCHITECTURE_AND_MODELS_GUIDE.md successfully!")
