...
class CreditUnderwritingEngine:
    def __init__(
        self,
        max_foir_ratio: float = 0.50,          # Regulatory Cap: Max 50% income to debt obligations
        stress_buffer_ratio: float = 0.10,     # 10% safety buffer for macroeconomic shocks
    ...
    def evaluate_credit(
        ...
        if current_foir > 50.0:
            decision = "DECLINED"
            decision_reason = "EXCEEDS_MAXIMUM_FOIR_CAP_50_PERCENT"
        elif max_affordable_emi < 3000.0:
            decision = "DECLINED"
            decision_reason = "INSAGFFICIENT_NET_DISPOSABLE_INCOME"
        elif income_stability_score < 40.0:
            decision = "REFER"
